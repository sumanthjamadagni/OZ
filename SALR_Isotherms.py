...
#Ur_ref = Potentials.SRLRPotential(r,sig=1.0, eps=5.0, m=100, n=50, d=2, A = 0.40)
#Ur = Potentials.SALRPotential(r,sig=1.0, eps=5.0, m=100, n=50, d=2, A = 0.40)
...
drho = 0.05
rhoArray = np.arange(0.005, 0.80, drho)
#rhoArray = np.arange(1.00, 0.60, -drho)
PlotFreq = 1 #int(0.10/drho)
...
for irho in range(nrho):
    rho = rhoArray[irho]
    #Flag, hr, cr, er, hk, Sk = HNC.OZSolver_HNC_Iterative(r, k, Ur_ref, rho, kT=T, maxiter=10000, w_old_start=0.50,w_old_max=0.99,tol=1e-8, cr_guess=cr_guess)    
...
#P_compressibility = PP_Functions.Calc_Pcompressibility(rho_converged, Compressibility, opt='cum',kT=T, B2=B2)
...
#Fig3.plot(rho_converged, Pressures, 'b-s', label='Virial')
#Fig3.text(0.10, 0.70, "B$_2$=" + str(np.round(B2,2)), transform=Fig3.transAxes, fontsize=FS)
#Fig3.plot(rho_converged, P_gas, 'g-d', label='IG + B2')
#Fig3.plot(rho_converged[1:], P_compressibility, 'r-d', label='Compressibility')
#Fig3.legend(loc='upper left')

Fig4.plot(rho_converged, Compressibility, 'r-d')
Fig.tight_layout()

Figs2.plot(rho_converged,ExcessEntropy, 'r-d')
